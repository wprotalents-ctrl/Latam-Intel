...
import { getGemini } from "./_lib/gemini.js";
...
async function handleBrief(req: VercelRequest, res: VercelResponse) {
  try {
    ...
    const r = await model.generateContent(
      `Write a 150-word LATAM AI workforce brief. Direct, no filler, data-driven.\nNEWS: ${newsContext}\nTRENDS: ${trendsContext}`
    );
    res.json({ brief: r.response.text() });
  } catch (e: any) {
