// scripts/check-salary.js — one-off diagnostic, can be deleted
...
(async () => {
  ...
  console.log('=== salary_benchmarks (live) — total rows:', live.data?.length || 0);
  if (live.data) {
  ...
  const prop = await sb
    ...
    .limit(1000);
  console.log('\n=== salary_benchmarks_proposed — total rows:', prop.data?.length || 0);
  if (prop.data) {
