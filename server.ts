...
async function startServer() {
  try {
    ...
    db = getFirestore("ai-studio-9279f702-e40c-457b-b350-29aa2957fe9a");
    ...
    try {
      Client.init(process.env.COINBASE_COMMERCE_API_KEY || "placeholder");
      Charge = resources.Charge;
    } catch (e) {
      console.error("Coinbase initialization failed:", e);
    }

    // Initialize Resend
    resend = new Resend(process.env.RESEND_API_KEY || "re_placeholder");
    ...
    app.post("/api/create-crypto-charge", async (req, res) => {
      ...
      try {
        if (!Charge) throw new Error("Coinbase Charge service not initialized");
